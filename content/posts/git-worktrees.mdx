...
<Warning>
    ...
    out `main{:txt}` in one worktree, an error like "fatal: 'main' is already checked
...
<Info>
    Take a look at the `.git{:txt}` file in your worktrees. Unlike what you would find
    in normal clones, it is a file that links to your original `.git{:txt}` file.
</Info>
