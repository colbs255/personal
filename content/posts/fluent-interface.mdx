...
<Note>
    Method chaining and fluent interfaces are not the same thing.
...
<Note>
    The return type of the method is the interface `AssertState` NOT the implementation `StringAsserter`.
...
<Note>
    JOOQ defines constants for table and column names.
...
1. Implement all your interfaces in your class. Each method mutates something in your class and then returns `this`.
