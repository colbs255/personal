...
That's the hard part! Let's finish this wih a nice `assertThat{:java}` method for the client:
...
<Note>
    The return type of the method is the interface `AssertState{:java}` NOT the
    implementation `StringAsserter{:java}`. You do not want to expose your
